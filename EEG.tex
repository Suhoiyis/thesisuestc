...
>>>>>>> 3e18085 (commit for final)
\usepackage{amsmath}   % 如果你之前没加这个包，需要加上，它提供了 \numberwithin 命令

% 1. 让图片计数器随 section (节) 变化，每新的一节重新计数
\numberwithin{figure}{section}

% 2. 让表格计数器随 section (节) 变化，每新的一节重新计数
...
\begin{abstract}
\noindent\textbf{【摘要】} 脑电图（Electroencephalogram, EEG）信号具备样本规模有限、信噪比偏低、非平稳性明显以及个体间差异显著等特点，这些特共同制约了深度学习模型在处理此类数据时的泛化性能。因此，数据增强技术成为提升模型鲁棒性与适应能力的关键途径。本文从软件工程与机器学习相结合的跨学科视角，对EEG数据增强技术的演进脉络进行了系统梳理，并将其发展历程划分为四个递进的阶段。演进过程起始于依赖信号处理理论的确定性增强方法，随后过渡至基于样本插值的正则化增强策略，进而发展为以生成对抗网络及扩散模型为代表的概率生成增强技术，最终演进至当前以前沿的自监督学习与对比学习为核心的隐式增强范式。本文的核心学术贡献主要体现在以下四个方面：其一，本文提出了一个能够统一描述该领域技术发展的四范式分类框架；其二，本文揭示了其中存在的三大关键工程挑战，即表征物理约束的有效融入、系统性评估维度的相对缺失，以及实际部署中资源消耗与性能的权衡问题；其三，本文通过设定极端小样本、类别不平衡、高个体差异性以及实时性约束四类典型的数据困境与应用场景，实证性地剖析并界定了不同增强方法的有效性与适用边界；其四，本文进一步展望了未来若干关键研究方向，包括轻量化生成技术、多模态信息引导增强以及标准化评估体系的构建，旨在推动EEG数据增强技术从依赖经验的技巧性实践，转变为具备可验证性与可部署性的系统性工程实践。
\\[0.5em]
\noindent\textbf{关键词：} 脑电图；数据增强；生成模型；自监督学习；工程评估;
\end{abstract}

\begin{abstract}
\noindent\textbf{Abstract:} The inherent characteristics of Electroencephalogram (EEG) signals—such as small sample sizes, low signal-to-noise ratio, non-stationarity, and signifi-cant inter-subject variability—severely limit the generalization capability of deep learning models, making data augmentation a key technology for enhancing robustness. From the cross-perspective of software engineering and machine learning,this paper systematically reviews the four-stage evolutionary trajectory of EEG data augmentation: from deterministic augmentation based on signal processing,to regularization augmentation based on sample interpolation, then to probabilis-tic generative augmentation based on generative models (GAN/Diffusion Models),and finally developing into an implicit augmentation paradigm centered on self-supervised and contrastive learning. The core contributions of this paper include:(1) proposing a unified four-paradigm classification framework; (2) revealing three major engineering challenges: representation physical constraints, lack of evalu-ation dimensions, and deployment resource trade-offs; (3) empirically analyzing the effectiveness boundaries of various augmentation methods through four typical data dilemmas: extreme small samples, class imbalance, high inter-subject vari-ability, and real-time constraints; (4) looking forward to future directions such as lightweight generation, multimodal guidance, and standardized evaluation, promot-ing the transition of EEG data augmentation from empirical tricks to verifiable and deployable engineering practices .
\\[0.5em]
...
脑电作为一种非侵入式的神经生理监测技术，以其毫秒级的高时间分辨率特性，成为探测大脑活动的重要工具。该技术在脑机接口系统、神经系统疾病辅助诊断以及认知机理研究等诸多领域，均发挥着不可替代的关键作用。

然而，基于EEG信号构建的深度学习模型在迈向实际应用时，首先面临着一系列源于数据本质属性的严峻挑战。其中，EEG信号固有的低信噪比与显著的非平稳性构成核心难点。具体而言，大脑皮层产生的微弱电信号极易受到来自眼动、肌肉收缩等多种生理伪迹的干扰与污染\cite{heDataAugmentationDeep2021}。与此同时，信号的统计特征会随被试者的生理状态、注意力水平及实验环境等因素而随时间动态变化。这种敏感且易变的 脆弱 属性，对模型的鲁棒性与泛化能力提出了极高的要求。

其次，个体间以及同一受试者不同会话间存在的显著差异，构成了模型实现跨主体与跨会话泛化的核心障碍。具体而言，不同受试者的大脑在解剖结构、功能连接模式等方面均存在固有差异。此外，即便是同一个体，其在不同实验会话中的心理状态、注意力集中度乃至环境因素的变化，均可能导致所采集EEG数据的概率分布发生漂移\cite{liuDomainAdaptationCrossSubject2021,WangXinRanMianXiangKuaShiDuanNaoWenShiBieDeYuFanHuaFangFaYanJiu2025}。这种由主体内与主体间变异性所导致的数据分布不一致性，使得在单一被试或有限会话数据上训练并表现优异的模型，在面对新被试或新会话数据时，其性能往往会出现显著衰退。

更为根本的挑战在于专家标注数据的极端稀缺性及其高昂的获取成本。与ImageNet等具备海量标注资源的计算机视觉任务不同，EEG数据的采集依赖于专业设备与受控环境，其数据标注需依赖领域专家\cite{XiaXiuXinJiYuShenDuXueXiDeXiuJueWeiJueYuLianJueNaoDianShuJuShiBieFangFaYanJiu2025,HuWenRongJiYuShuJuZengQiangHeShenDuShenJingWangLuoDeDianXianJianCeYanJiu2025}。以运动想象或情绪识别等任务为例，单次试验通常仅能对应一个有效标签，这直接导致多数数据集仅包含数十至数百个样本\cite{ZhangJiaZhengJiYuNaoJiDianRongHeDeDanCeZhiTiYunDongXiangXiangJieMaFangFaYanJiu2025}。此种小样本困境使得复杂的深度网络极易因参数过多而陷入过拟合，难以捕获具有稳健泛化能力的本质特征。


\subsection{数据增强的必要性与目标}
面对上述挑战，数据增强技术从 以数据为中心的人工智能 这一视角出发\cite{moghadamDataCentricAIEEGBased2025}，提供了一条关键路径。其核心逻辑并非如传统去噪或滤波方法那样旨在提升单一样本的质量，而是通过系统性地扩充与变换训练数据集，来更充分地覆盖数据的潜在分布。这种方法旨在构建一个在分布层面上更具多样性与包容性的训练环境，从而增强模型学习的鲁棒性。
...
\begin{enumerate}[label={（\arabic*）}, leftmargin=*]
    ...
    \item \textbf{增强模型鲁棒性}：对训练数据施加可控的扰动或人工噪声，引导模型学习对输入数据中非关键性微小变差保持不敏感的特性，提升模型在实际应用场景中面对未见的噪声、伪迹或分布外样本时的稳定性与可靠性。
\end{enumerate}

自动化数据增强策略~\cite{cubukAutoAugmentLearningAugmentation2019}的出现，将数据增强从一个依赖于专家经验的技巧变为一个可学习的模型组件。

\subsection{本文组织结构}
本文旨在融合软件工程与机器学习的跨学科视角，系统性地梳理EEG数据增强技术的演进脉络、前沿进展及其工程实践。第2章将依据技术范式的演进顺序，依次剖析确定性增强、正则化增强、概率生成增强以及隐式增强这四类核心方法；第3章重点关注表征物理约束、评估体系缺失以及工程部署瓶颈三大现实挑战；第4章则通过构建小样本、类别不平衡、高个体差异及实时性约束四类典型数据困境，检验不同增强策略的效能边界。第5章归纳总结了技术演进的内在规律，并进一步展望了轻量化生成、多模态信息引导以及标准化评估体系等关键未来研究方向。为清晰呈现本文的整体逻辑框架，图\ref{pic:roadmap}系统性地描绘了EEG数据增强技术的范式演进路径及其与工程实践环节的关联。
...
为清晰刻画技术演进脉络，本文将 EEG 数据增强方法划分为四大范式，其核心差异在于增强操作的生成性、语义性与集成深度：

\begin{enumerate}[label={（\arabic*）}, leftmargin=*]
    \item \textbf{确定性增强}：通过预设的信号处理算子（如时移、频带交换）对单个样本进行可逆变换，不引入新语义，计算高效且生理可解释性强；

    \item \textbf{正则化增强}：通过对两个或多个样本进行线性/非线性插值（如 Mixup），在输入或特征空间构造 软样本 ，以平滑决策边界，本质上是一种正则化策略；

    \item \textbf{概率生成增强}：通过学习真实数据分布 $P(X,Y)$ ，利用生成模型（如 GAN、扩散模型）合成具有新语义的逼真样本，显著扩充数据多样性；
...
基于信号处理的确定性增强，是脑电图分析领域最早得到应用的数据增强范式，也是当前工业实践中最为稳健可靠的预处理与增强手段之一。该方法的核心在于不依赖于对数据分布的统计建模，而是依据脑电信号在时域、频域及时频域中所呈现的已知物理特性，通过预设的几何变换或确定性信号处理算子来生成新样本。Moghadam等人\cite{moghadamDataCentricAIEEGBased2025}所倡导的 以数据为中心的人工智能 理念与此高度契合，其在SEED-VII数据集上的实验表明，即使不改变模型架构，仅通过参与者引导的伪迹滤除与系统化的确定性增强，即可获得优于某些复杂模型的性能。这一发现进一步巩固了此类方法作为整个技术体系基石的工程地位。

在时域层面，滑动窗口及其各类变体技术是应对长程信号分段与数据扩充需求的首选方法。叶晓雅等人\cite{YeXiaoYaEEGMFNetGuaYongYuNaoDianXinHaoFenXiDeQingLiangJiDuoFenZhiRongHeWangLuo2025}在构建轻量级多分支融合网络时，便借助滑动窗口技术有效缓解了脑机接口数据集样本数量不足的局限，从而提升了模型对运动想象信号的解码性能。谷学静等人\cite{GuXueJingRongHeZhuYiLiDeShuangFenZhiShiKongJuanJiNaoDianShiBieWangLuo2025}在其设计的双分支时空卷积网络中，引入了一种改进的滑动窗口策略，并结合注意力机制，共同强化了对信号的特征表达能力。同时，Zhou等人\cite{zhouAutoAugHARAutomatedData2024}通过将滑动窗口技术与时间卷积网络相结合，有效增加了训练样本的时序多样性，从而显著提升了运动想象任务的解码性能。另一方面，针对稳态视觉诱发电位（SSVEP）信号对数据长度高度敏感的特性，Zhang等人\cite{ZhangSiYuanMianXiangWenTaiShiJueYouFaDianWeiDeNaoJieMaGeTiGuaPeiXingJiShuYanJiu2025}提出了一种基于改进的任务相关成分分析（TRCA）框架的数据长度适配方法。该方法结合了锁相时移的数据增强策略，有效缓解了被试内及跨被试神经响应存在的差异性所导致的模型泛化难题

在频域与变换域的增强策略层面，核心目标在于通过模拟信号在频谱上的合理扰动或特征重构，提升模型对频带偏移等变化的适应能力。为达成此目标，Du等人\cite{duDataAugmentationMethod2024}提出了一种时频域联合增强的多尺度特征融合算法。该方法首先对经独立成分分析（ICA）去噪后的信号，分别施加时域与频域的增强处理，随后从中提取小波包能量特征进行融合。这一综合方案有效缓解了因样本量有限而导致的分类性能波动问题。还有Zheng等人\cite{zhengMotorImageryEEG2025}提出的一种基于小波包分解的合成增强方法。该方法首先利用小波包分解将原始信号解构为低方差的稳定分量与高方差的动态分量。在此基础上，通过对不同子带成分进行有选择的交换与重组，能够生成在保留事件相关同步关键特征的同时，有效模拟信号内在变异的合成样本。这一方法在显著减少所需传感器数量的情况下，仍能维持较高的分类精度。

此外，信号统计特性分析与多维重构技术也被引入增强流程。Liao等人\cite{liaoEEGDataAugmentation2025}提出了一种基于高斯混合模型的增强方法，其核心在于分解同类样本，并交换由概率系数加权的特征矩阵列向量，所生成的新样本在t-SNE可视化中呈现出更为紧凑的时空分布。在涉及多感官融合的研究中，Xia等人\cite{XiaXiuXinJiYuShenDuXueXiDeXiuJueWeiJueYuLianJueNaoDianShuJuShiBieFangFaYanJiu2025}针对味觉刺激EEG数据稀缺的难题，提出了一种时空重构数据增强方法，该方法通过对信号的时空维度特征进行解构与重组，有效丰富了训练数据的多样性。确定性增强往往是高性能现代架构的前置基础，例如 Li 等\cite{LiXiangJiYuCNNYuTransformerDeNaoDianJieMaYanJiu2025}提出的 RCA-Conformer 架构，虽然利用了 Transformer 的全局建模能力，但仍需依赖稳健的特征输入来防止过拟合，确定性增强在此类混合模型中至关重要。

尽管确定性增强具备高稳定性与低计算开销的优势，但其本质是对原始信号流形的局部扰动。在极端小样本场景或需要模拟显著个体差异的任务中，这种方法难以生成具有新语义信息的样本，泛化能力提升有限。因此，其更适合作为其他增强范式的基础预处理模块，而非独立解决方案。表 \ref{tab:2.1}   进一步总结并对比了主流确定性增强方法的操作逻辑与适用场景。
...
为了突破单样本变换的局限性，技术范式向样本间插值演进，该范式引入了混合（Mixup）的概念，假设在特征空间或输入空间中，样本间的线性插值点仍然具有语义意义，从而通过软化标签实现正则化。

Mixup 及其变体在处理类别不平衡和提升鲁棒性方面表现优异。He 等\cite{HeHuanHuanJiYuNaoDianXinHaoDeShuJuZengQiangYuQingXuShiBieYanJiu2025}在癫痫发作检测的系统性研究中，深入评估了 Mixup、SuperMix 和 Co-MixUp 三种策略，发现Co-MixUp 能最大化混合样本与源样本，极大地缓解了EEG 监测中严重的类别不平衡问题。在运动想象领域，Ding 等\cite{duDataAugmentationMethod2024}则将Mixup 策略引入了深度卷积网络MixedCNN，实验表明经连续小波变换（CWT）处理后的样本结合 Mixup，能使分类准确率提升至 93.57\%。此外，Han 等\cite{hanMSRLNetMultiSourceFusion2025}在针对 ADHD 的 MSRLNet 网络中，引入了特征级数据增强，在 CNN-GRU 提取的高维特征空间内进行扰动与混合，进一步验证了在特征流形上进行增强的有效性。

多模态融合也是一种广义的信息增强。Liao 等\cite{liaoEEGDataAugmentation2025}提出的脑肌电（EEG-EMG）融合方法，通过注意力机制动态分配双模态权重，利用肌电信号增强了脑电信号在单侧肢体运动解码中的特征表达能力，弥补了单一模态的信息缺失。针对跨被试场景下的分布差异，Liu 等\cite{liGMSSGraphBasedMultiTask2023}提出了基于被试聚类的域适应增强策略，将相似情感响应模式的被试聚类并在类内进行源域信息的迁移，这种策略在逻辑上类似于在特定的流形上进行样本插值。

随着增强策略的增多，自动化数据增强应运而生。Cubuk 等\cite{cubukAutoAugmentLearningAugmentation2019}在图像领域的开创性工作启发了 EEG 研究者。Rommel 等\cite{rommelCADDAClasswiseAutomatic2022}提出了 CADDA（类可微自动数据增强），针对 EEG 信号特性设计了可微的增强操作搜索空间，实现了针对特定任务的最优策略自动搜索。类似地，Zhou 等提出的 AutoAugHAR 框架，设计了针对传感器数据的两阶段梯度优化流程，证明了自动化策略在跨被试泛化中具有超越人工设计策略的潜力。表\ref{tab:2.2}对混合插值与自动增强这两类正则化策略的原理与效果进行了对比展示。
% 图4///////////////////////////////////////////////////////
...
概率生成增强旨在学习真实 EEG 数据的联合分布 $P(X,Y)$，并通过采样生成新样本。该范式主要包括生成对抗网络与扩散模型两大技术路线，他们在训练稳定性、生成质量与计算成本上存在显著差异。


\subsubsection{生成对抗网络（GAN）}
GAN 及其变体是早期 EEG 生成的主流框架。Habashi 等\cite{habashiGenerativeAdversarialNetworks2023}的综述系统梳理了GAN 在运动想象、P300 及癫痫检测中的应用，Du 等\cite{duDataAugmentationMethod2024}提出基于 DCGAN-GP 的方法，将 EEG 转为二维时频图进行生成；Song 等\cite{songEEGGANNetEnhancingEEG2024} 进一步结合条件GAN 与 SE 注意力机制，实现类别可控生成，为提升特征解耦能力，InfoGAN 被引入MI 任务\cite{XiaoNanJiYuXinXiZuiDaHuaShengChengDuiKangWangLuoDeYunDongXiangXiangNaoDianXinHaoShuJuZengQiang2025}，此外，VAE-GAN 混合架构（如 SSVAE-CGAN\cite{YuanKaiFengJiYuBanJianDuVAEHeCGANDeYunDongXiangXiangNaoDianXinHaoFenLeiQi2025} 、DEVAE-GAN\cite{tianDualEncoderVAEGANSpatiotemporal2023}）通过双编码器分别建模时空潜在变量，在情感 EEG 生成中展现出更强的分布覆盖能力。


然而，GAN在EEG生成中仍面临训练不稳定、梯度消失、评估困难三大瓶颈，尤其在高维时序信号上易出现高频噪声或相位失真，或在小批量EEG数据上易出现收敛困难。

\subsubsection{扩散模型（Diffusion Models）}
近年来，扩散模型凭借其训练稳定性高、样本多样性好、频谱保真度强等优势，逐渐成为 EEG 生成的新范式。Zhang 等构建基于 DDPM 的生成框架，在视觉刺激解码任，务中提升了9.56\%的准确率 ；Moghadam 等\cite{moghadamDataCentricAIEEGBased2025}提出的卷积注意力扩散模型（CADM）专门针对抑郁症检测中的小样本问题，生成的时频特征能有效增强 Transformer 的鲁棒性。在跨模态任务中，离散扩散模型首次被用于脑电到语音合成\cite{FangYuanJiYuShenDuXueXiDeEEGNaoDianTuXinHaoYuYinHeCheng}，克服了非侵入式信号质量低的挑战。
...
% =============== 第3章 ===============
\section{关键技术挑战与工程实践}
EEG 数据增强技术虽然在理论上缓解了小样本困境，但要将其转化为可靠的工程能力，仍需跨越从实验室到规模化部署的鸿沟。当前制约技术落地的核心矛盾，已从“能否提升准确率”转向“是否安全、可信、高效”\cite{PengLeiNaoDianXinHaoQingXuShiBieGuanJianJiShuYanJiuJinZhan2024}。本节将系统说明遇到的三大关键挑战：增强操作必须满足的物理一致性约束、评估体系缺失导致的质量误判风险，以及工程部署中的典型陷阱，并提出可操作的应对策略。
...
首要的评估维度是生理频谱保真度，高质量的增强样本必须在频谱特性上复现真实脑电，生成样本必须保持与真实 EEG 一致的ERD/ERS模式，否则模型将学习到错误的特征\cite{YuanKaiFengJiYuBanJianDuVAEHeCGANDeYunDongXiangXiangNaoDianXinHaoFenLeiQi2025}。实验证实，相比标准 MixUp 导致的频谱失真，Co-MixUp 等改进方法能更好地保持 \textdelta 和 \texttheta 频段的功率谱密度\cite{tianDualEncoderVAEGANSpatiotemporal2023,QiaoYingXiaoJiYuDuoWeiTeZhengTuHeShenDuXueXiDeNaoDianXinHaoQingXuShiBieFangFaYanJiu2025}。具体可采用功率谱密度均方误差（PSD-MSE）作为量化指标，其计算可基于MNE-Python的多锥度谱估计函数。

我们还需关注整体数据集的分布一致性 ，这时直接套用图像领域的 FID 指标会导致评估偏差，更合理的做法是采用针对 EEG 特性改进的指标（如基于 EEGNet 特征的 EEG-FID），或使用切片 Wasserstein 距离（SWD）来捕捉时频特性的分布差异\cite{SongChunNingJiYuShenDuXueXiDeYunDongXiangXiangNaoDianXinHaoShiBieFangFa2022,YuanKaiFengJiYuBanJianDuVAEHeCGANDeYunDongXiangXiangNaoDianXinHaoFenLeiQi2025}。

最终，这些指标应被整合到一起，在关注分类性能增益的同时同步监控频谱一致性、样本多样性及训练稳定性\cite{ZhangSiYuanMianXiangWenTaiShiJueYouFaDianWeiDeNaoJieMaGeTiGuaPeiXingJiShuYanJiu2025}。图\ref{pic:3.2}展示了这一评估框架，只有当生理保真度与分布一致性达标后，下游任务的性能提升才具有实际意义，否则高准确率可能源于模型对生成伪迹的过拟合。
...
首先是建立严格的数据隔离机制。数据泄露是模型评估失效的重要原因，最常见的错误是在数据划分前进行全局增强，因此工程实践必须遵循先划分、后增强的原则，确保每个原始样本及其增强子片段严格限制在单一数据集中\cite{ZhangJiaZhengJiYuNaoJiDianRongHeDeDanCeZhiTiYunDongXiangXiangJieMaFangFaYanJiu2025}，在跨被试场景必须确保同一被试的数据不会同时出现在正负样本对中或通过交叉验证设计确保受试者轮流作为目标域\cite{yeSemiSupervisedDualStreamSelfAttentive2025}。

EEG 信号的类内多样性有限，这意味着增强规模并非越大越好，多项研究证实，\cite{HuWenRongJiYuShuJuZengQiangHeShenDuShenJingWangLuoDeDianXianJianCeYanJiu2025}生成样本比例存在一个最优值（通常为原始数据的 70\% 左右），一旦超过此阈值，过度增强会导致模式崩溃，引发样本同质化，降低模型的泛化性能\cite{ZhengMinMinYiZhongDuoChiDuTeZhengRongHeYunDongXiangXiangNaoDianXinHaoFenLeiSuanFaJiYuShiPinYuShuJuZengQiang2025}，动态调整增强强度能有效解决这个问题\cite{SongFeiYuJiYuShiKongFenXiHeDuiBiXueXiDeYunDongXiangXiangNaoDianXinHaoJieMaYanJiu2025,LiJiDianXianNaoDianDeDuiBiXueXiTeZhengBiaoShiJiYongYuZhiXianQuShiBieDeNaoDianYuXunLianMoXingYanJiu2025}，同时可引入样本多样性监控机制，定期计算生成样本在 t-SNE 空间的聚类熵，低于阈值则自动降低生成强度。

最后，生成式模型虽然生成内容质量高，但在实时系统中经常难以落地\cite{WangZiKaiYongYuYiYuZhengNaoDianShuJuZengQiangDeJuanJiZhuYiLiKuoSanMoXing2025}，可以采取非对称的轻量化策略和分阶段部署，在算法选择上优先采用计算成本仅为原始信号生\cite{WangXinRanMianXiangKuaShiDuanNaoWenShiBieDeYuFanHuaFangFaYanJiu2025}成 40\% 的特征空间增强（如 MixReg）或在架构层面采用离线重生成、在线轻增强的两阶段部署方案\cite{WangZiKaiYongYuYiYuZhengNaoDianShuJuZengQiangDeJuanJiZhuYiLiKuoSanMoXing2025}，同时在训练流水线中引入针对增强操作的结果缓存机制能有效降低重复计算负荷\cite{rommelCADDAClasswiseAutomatic2022}，在有限算力下实现性能与效率的平衡。
...
本文第 2 节系统梳理了 EEG 数据增强的技术范式，第 3 节提出了评估与工程准则，但最终仍需通过真实任务场景验证其有效性，但是有效性并非普适，不同增强方法在不同数据困境下表现迥异，本节不再按应用领域划分，而是依据四类典型数据挑战组织案例反向揭示各类增强范式的适用边界与作用机制。



\subsection{极端小样本场景下的生成式增强优势}
当每类训练样本少于50个时（常见于运动想象、罕见认知任务），传统模型极易过拟合。此时，概率生成增强展现出重要价值。肖楠等\cite{XiaoNanJiYuXinXiZuiDaHuaShengChengDuiKangWangLuoDeYunDongXiangXiangNaoDianXinHaoShuJuZengQiang2025}提出的DCIMGAN（基于InfoGAN）通过最大化生成信号与潜在变量的互信息，实现了对MI-EEG中ERD/ERS特征的可控生成。在BCI Competition IV 2a数据集（每类仅约72 trials）上，该方法使EEGNet的平均准确率提升6.2\%，显著优于滑动窗口等确定性增强。类似地，夏秀鑫\cite{XiaXiuXinJiYuShenDuXueXiDeXiuJueWeiJueYuLianJueNaoDianShuJuShiBieFangFaYanJiu2025}在嗅觉、味觉与联觉等超小样本认知任务（部分类别<20样本）中，采用时空重构增强有效提升了识别性能，证实生成式方法能突破原始数据流形边界。

扩散模型在离线小样本场景中潜力巨大，汪子凯等\cite{WangZiKaiYongYuYiYuZhengNaoDianShuJuZengQiangDeJuanJiZhuYiLiKuoSanMoXing2025}针对抑郁症EEG 数据稀缺问题，提出卷积注意力扩散模型（CADM），生成的时频特征在 MODMA 数据集上使Vision Transformer 准确率提升 8.7\%。


\subsection{严重类别不平衡场景中的正则化与定向生成}
在癫痫检测等任务中，病理事件占比常低于 1\%，导致模型偏向多数类，针对性增强少数类成为关键，胡文蓉\cite{HuWenRongJiYuShuJuZengQiangHeShenDuShenJingWangLuoDeDianXianJianCeYanJiu2025}通过过采样与生成病理样本，使 1DCNN-BiLSTM 模型在CHB-MIT 数据集上将发作检测 F1-score 从 0.62 提升至 0.81，He 等\cite{heDataAugmentationDeep2021}系统比较 Mixup、SuperMix 与 Co-MixUp 后发现，Co-MixUp 通过显著性图引导插值权重，能更好保留癫痫高频振荡特征，在长程监测中 正 确 率 提升达 9.3\%。
