...
多模态融合也可被视为一种广义的信息增强。Liao等\cite{liaoEEGDataAugmentation2025}提出的脑肌电（EEG-EMG）融合方法，通过注意力机制动态分配双模态权重，实际上是利用肌电信号增强了脑电信号在单侧肢体运动解码中的特征表达能力，弥补了单一模态的信息缺失。针对跨被试场景下的分布差异，Liu等\cite{LiuGuoRuiZiGuaYingEMDShuJuZengQiangHeHunHeChiDuCNNDeNaoDianXinHaoShiBie2022}提出了基于被试聚类（Subject Clustering）的域适应增强策略（DASC），通过将相似情感响应模式的被试聚类并在类内进行源域信息的灵活迁移，这种策略在逻辑上类似于在特定的“流形”上进行样本插值。
